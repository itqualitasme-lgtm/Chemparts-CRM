...
/** One of a client's still-actionable requests, shown in the response form. */
...
export type PriceRow = {
  ...
  siblings: RespondItem[] // all of this client's actionable requests (incl. this one)
  qty: number
  message: string | null
...
const actionable = (s: string) => s === 'OPEN' || s === 'AWAITING_INFO'
const selectCls =
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="w-full min-w-[760px] text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-3 py-10 text-center text-slate-500">No price requests match these filters.</td>
              </tr>
            ) : (
    ...
    <>
      <tr className="border-b border-slate-100 last:border-0 align-middle">
        <td className="px-3 py-2">
          <span className={`rounded px-2 py-0.5 text-xs font-medium ${STATUS_BADGE[r.status]}`}>{STATUS_LABEL[r.status] ?? r.status}</span>
        </td>
        ...
        <td className="px-3 py-2 text-right">
          {r.siblings.length > 1 && actionable(r.status) ? (
            <span className="mr-1 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">+{r.siblings.length - 1} more</span>
          ) : null}
          <button
            type="button"
          ...
          >
            {actionable(r.status) ? 'Respond' : 'View'}
          </button>
        </td>
      </tr>
      <DetailModal
        ...
        header={
          <>
            <span className={`rounded px-2 py-0.5 text-xs font-medium ${STATUS_BADGE[r.status]}`}>{STATUS_LABEL[r.status] ?? r.status}</span>
            <span className="text-sm font-semibold text-slate-900">{r.requester}</span>
      ...
      >
        ...
        <div className="rounded-lg bg-white ring-1 ring-slate-200">
          <div className="border-b border-slate-100 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Client</div>
        ...
        {r.message ? (
          <div className="rounded-lg bg-white ring-1 ring-slate-200">
            <div className="border-b border-slate-100 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Their message</div>
            <p className="whitespace-pre-line px-3 py-2.5 text-sm leading-relaxed text-slate-600">{r.message}</p>
          </div>
        ) : null}

        {actionable(r.status) ? (
          <RespondForm items={r.siblings} clientEmail={r.clientEmail} />
        ) : (
