...
// Importer for the Lumex Instruments (lumexinstruments.com) catalogue — mercury /
...
const MANIFEST = process.env.LUMEX_MANIFEST as string
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'lumex' }, select: { id: true } })
  if (!brand) throw new Error('Lumex brand not found')
  ...
  const withSlug = rows.map((r) => {
    const base = slugify(`lumex ${r.slug}`).slice(0, 70).replace(/-+$/, '')
    return { ...r, pslug: foreign.has(base) ? `${base}-lumex` : base }
  })
  ...
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Lumex products:', removable.length) }
  }
    ...
    await Promise.all(withSlug.slice(i, i + 4).map(async (r) => {
      if (APPLY) {
        ...
        for (let n = 0; n < gallery.length; n++) {
          ...
          const url = await store(gallery[n], `lumex/products/${r.pslug}-${n}.${ext}`)
        ...
        await db.product.upsert({
          where: { slug: r.pslug },
          create: {
            slug: r.pslug, name: r.name, brandId: brand.id, categoryId: catId[r.catSlug] ?? null, type: 'EQUIPMENT',
