...
// Importer for the Chromos (chromosgc.com) catalogue — laboratory & process gas
// chromatographs, specialized analyzers, integrated analyzer shelters, and lab
// metalwork/furniture. Content lives in a Tilda site; product text + hero photos
// were extracted per page. Replaces the 2 legacy placeholder rows. Slug-guarded.

const MANIFEST = process.env.CHROMOS_MANIFEST as string
...
type Row = { name: string; category: string; catSlug: string; desc: string; image: string | null }
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'chromos' }, select: { id: true } })
  if (!brand) throw new Error('Chromos brand not found')
  ...
  const withSlug = rows.map((r) => {
    ...
    return { ...r, slug: foreign.has(base) ? `${base}-chromos` : base }
  })
  const newSlugs = new Set(withSlug.map((r) => r.slug))

  // Replace old (unreferenced) Chromos rows — the 2 legacy placeholders.
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Chromos products:', removable.length) }
  }
  ...
  for (const r of withSlug) {
    if (APPLY) {
      ...
      if (r.image) {
        ...
        uploaded = await store(r.image, `chromos/products/${r.slug}.${ext}`)
        if (uploaded) imgs++
      }
