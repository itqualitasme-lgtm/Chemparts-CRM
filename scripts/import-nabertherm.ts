...
// Importer for the Nabertherm Instruments (nabertherminstruments.com) catalogue — mercury /
...
const MANIFEST = process.env.NABERTHERM_MANIFEST as string
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'nabertherm' }, select: { id: true } })
  if (!brand) throw new Error('Nabertherm brand not found')
  ...
  // Nabertherm's method-name slugs are near-identical past 70 chars (…-tv4000 vs
  ...
  const withSlug = rows.map((r) => {
    const full = slugify(`nabertherm ${r.slug}`)
  ...
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Nabertherm products:', removable.length) }
  }
    ...
    await Promise.all(withSlug.slice(i, i + 4).map(async (r) => {
      if (APPLY) {
        ...
        for (let n = 0; n < gallery.length; n++) {
          ...
          const url = await store(gallery[n], `nabertherm/products/${r.pslug}-${n}.${ext}`)
