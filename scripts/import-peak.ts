...
// Importer for the Peak Instruments Instruments (peakinstruments.com) catalogue — mercury /
...
const MANIFEST = process.env.PEAK_MANIFEST as string
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'peak-instruments' }, select: { id: true } })
  if (!brand) throw new Error('Peak Instruments brand not found')
  ...
  // Peak Instruments's method-name slugs are near-identical past 70 chars (…-tv4000 vs
  ...
  const withSlug = rows.map((r) => {
    const full = slugify(`peak ${r.slug}`)
  ...
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Peak Instruments products:', removable.length) }
  }
    ...
    await Promise.all(withSlug.slice(i, i + 4).map(async (r) => {
      if (APPLY) {
        ...
        for (let n = 0; n < gallery.length; n++) {
          ...
          const url = await store(gallery[n], `peak/products/${r.pslug}-${n}.${ext}`)
