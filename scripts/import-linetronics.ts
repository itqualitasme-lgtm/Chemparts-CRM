...
// Importer for the Linetronics Instruments (linetronicsinstruments.com) catalogue — mercury /
...
const MANIFEST = process.env.LINETRONICS_MANIFEST as string
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'linetronics' }, select: { id: true } })
  if (!brand) throw new Error('Linetronics brand not found')
  ...
  // Linetronics's method-name slugs are near-identical past 70 chars (…-tv4000 vs
  ...
  const withSlug = rows.map((r) => {
    const full = slugify(`linetronics ${r.slug}`)
  ...
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Linetronics products:', removable.length) }
  }
  ...
  for (let i = 0; i < withSlug.length; i += 4) {
    await Promise.all(withSlug.slice(i, i + 4).map(async (r) => {
      if (APPLY) {
        const gallery = ((r.images && r.images.length) ? r.images : r.image ? [r.image] : []).slice(0, 4)
        const uploaded: string[] = []
        for (let n = 0; n < gallery.length; n++) {
          const ext = (gallery[n].split('.').pop() || 'jpg').split(/[?#]/)[0].toLowerCase()
          const url = await store(gallery[n], `linetronics/products/${r.pslug}-${n}.${ext}`)
