...
// Importer for the Fluitec (fluitec.com) oil-reliability range: condition-
...
const MANIFEST = process.env.FLUITEC_MANIFEST as string
...
async function main() {
  ...
  const brand = await db.brand.findFirst({ where: { slug: 'fluitec' }, select: { id: true } })
  if (!brand) throw new Error('Fluitec brand not found')
  ...
  const withSlug = rows.map((r) => {
    const base = slugify(`fluitec ${r.slug}`).slice(0, 70).replace(/-+$/, '')
    return { ...r, pslug: foreign.has(base) ? `${base}-fluitec` : base }
  })
  ...
  if (APPLY) {
    ...
    if (removable.length) { await db.product.deleteMany({ where: { id: { in: removable.map((p) => p.id) } } }); console.log('removed old Fluitec products:', removable.length) }
  }
  ...
  for (const r of withSlug) {
    if (APPLY) {
      ...
      if (r.image) {
        ...
        uploaded = await store(r.image, `fluitec/products/${r.pslug}.${ext}`)
        if (uploaded) imgs++
      }
